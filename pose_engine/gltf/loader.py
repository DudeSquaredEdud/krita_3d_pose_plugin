...
class TextureData:
    """glTF texture data."""
    name: str
    source_image: Optional[int]  # Index into images list
    sampler: Optional[int]  # Index into samplers list
...
class MaterialData:
    """glTF material data."""
    name: str
    base_color_factor: Tuple[float, float, float, float]  # RGBA
    base_color_texture: Optional[int]  # Index into textures
    metallic_factor: float
    roughness_factor: float
    metallic_roughness_texture: Optional[int]  # Index into textures
    normal_texture: Optional[int]  # Index into textures
...
class ImageData:
    ...
    name: str
    mime_type: str
    data: bytes  # Raw image data (PNG, JPEG, etc.)
...
class GLBLoader:
    ...
    def _parse_images(self, images_data: List[Dict], binary_buffer: bytes,
        ...
        """Parse glTF images and extract embedded data."""
        images = []
        ...
        for img in images_data:
            ...
            # Get image data from bufferView (embedded in GLB)
            if 'bufferView' in img:
                ...
                # External URI - not supported for now
                data = b''
    ...
    def get_texcoords(self, accessor_index: int) -> List[Tuple[float, float]]:
        """Get texture coordinates (VEC2) from an accessor."""
        raw_data = self._data.get_accessor_data(accessor_index)
        accessor = self._data.accessors[accessor_index]
